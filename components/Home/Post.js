        ...
        <View style={styles.leftPostFooter}>
            {postFooterIcons.slice(0, 3).map((icon, index) => (
                <Icon
                    ...
                    likedImageUrl={icon.likedImageURL}
                    name={icon.name}
                />
            ))}
        </View>
...
const Icon = ({ imgStyle, imgUrl, likedImageUrl, name }) => {
    ...
    return (
        <TouchableOpacity onPress={handleLike}>
            <Image 
                style={imgStyle} 
                source={{ 
                    uri: name === 'Like' ? (liked ? likedImageUrl : imgUrl) : imgUrl
                }} 
            />
        </TouchableOpacity>
    );
...
const Caption = ({ post }) => (
    <View style={{ marginTop: 4 }}>
        <Text>
            ...
            <Text style={{ color: 'white', fontWeight: '500' }}> {post.caption}</Text>
        </Text>
    </View>
);
